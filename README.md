
# 📁 Portfolio - BUT Réseaux & Télécoms

**Projet :** Réalisation d'un site web portfolio pour valoriser mes compétences techniques et mes projets académiques.

---

## 🚀 Présentation
Ce portfolio a été conçu dans le cadre de mon cursus en **BUT Réseaux & Télécoms**. Il sert de vitrine pour présenter mon parcours, mes certifications et les travaux pratiques réalisés (en réseaux, administration système et développement).

* **🌐 Voir le site :** <a href="https://antoninmll.github.io/portfolio/" target="_blank" rel="noopener noreferrer">https://antoninmll.github.io/portfolio/</a>
* **👤 Auteur :** Antonin Millet

---

## 🛠️ Stack Technique

| Domaine | Technologies |
| :--- | :--- |
| **Langages** | HTML5, CSS3, JavaScript, CSS |
| **Hébergement** | GitHub Pages |
| **Versionnage** | Git |

---

## 📋 Contenu du Portfolio

* **Accueil :** Introduction.
* **Compétences :** Focus sur les outils techniques (Cisco, Linux, Docker, Wireshark, etc.).
* **Projets :** Présentation des SAÉ (Situations d'Apprentissage et d'Évaluation) et projets personnels.
* **A propos :** présentation et contacts.
